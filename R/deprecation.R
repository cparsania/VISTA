...
.vista_deprecations <- function() {
  ...
  rows <- list(
    ...
    c("get_corr_heatmap", "cluster_by", "order_by", "1.2.0", "1.4.0", "1.6.0",
      "`cluster_by` names a column in the heatmap functions; renamed here to avoid the collision."),
    c("get_celltype_group_dotplot", "error", "errorbar", "1.2.0", "1.4.0", "1.6.0",
      "`error` is a logical control in validate_vista(); renamed here to avoid the collision."),
    c("get_deg_count_pieplot", "label", "label_type", "1.2.0", "1.4.0", "1.6.0",
      "`label` is logical elsewhere in VISTA; the enum moved to `label_type`."),
    c("get_deg_count_donutplot", "label", "label_type", "1.2.0", "1.4.0", "1.6.0",
      "`label` is logical elsewhere in VISTA; the enum moved to `label_type`."),
    c("get_expression_boxplot", "comparisons", "stat_comparisons", "1.2.0", "1.4.0", "1.6.0",
      "Avoids the clash with the comparisons() accessor and with DE contrasts."),
