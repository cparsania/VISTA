...
- log_transform:
  ...
  Logical; when `group_value` is used (and `value_column` is `NULL`),
  apply log2(x + 1) before coloring.
...
- If multiple `value_column`s are supplied, one plot is produced per
  ...
  are shown on the first occurrence of that column. Requires patchwork;
  otherwise a list of plots is returned.
