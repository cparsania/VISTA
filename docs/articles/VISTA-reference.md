...
- `group_denominator`: Denominator group(s), same length/order as
  numerator.
- `method`: `"deseq2"` (default) or `"edger"`.
...
- `p_value_type`: `"padj"` or `"pvalue"` (edgeR: `"FDR"`/`"PValue"`).
...
- `comparisons(x)`: DE results list.
- `deg_summary(x)`: DEG summary list.
...
- `get_enrichment_chord(...)`: chord diagram of gene–pathway
  relationships (requires circlize).
