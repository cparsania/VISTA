...
named vectors directly in VISTA metadata.

``` r

set_vista_group_colors <- function(v, color_map) {
  stopifnot(inherits(v, "VISTA"))
  meta <- S4Vectors::metadata(v)
  group_col <- meta$group$column
  groups <- unique(as.character(SummarizedExperiment::colData(v)[[group_col]]))

  missing_groups <- setdiff(groups, names(color_map))
  if (length(missing_groups) > 0) {
    stop("Missing group colors for: ", paste(missing_groups, collapse = ", "))
  }

  meta$group$colors <- color_map[groups]
  S4Vectors::metadata(v) <- meta
  v
}

set_vista_comparison_colors <- function(v, color_map) {
  stopifnot(inherits(v, "VISTA"))
  meta <- S4Vectors::metadata(v)
  comps <- names(comparisons(v))

  missing_comparisons <- setdiff(comps, names(color_map))
  if (length(missing_comparisons) > 0) {
    stop("Missing comparison colors for: ", paste(missing_comparisons, collapse = ", "))
  }

  meta$comparison$colors <- color_map[comps]
  S4Vectors::metadata(v) <- meta
  v
}
```
