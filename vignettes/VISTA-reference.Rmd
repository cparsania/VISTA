...
- `get_deg_count_pieplot(...)`: circular DEG composition summary (pie), with optional non-DE `"Other"` slice.
- `get_deg_count_donutplot(...)`: circular DEG composition summary (donut), with optional non-DE `"Other"` slice.
...
- `get_foldchange_heatmap(...)`: heatmap of log2FC; with no genes supplied, uses the top DE genes from each comparison.
...
- `get_genes_by_regulation(...)`: extract regulated gene sets for a comparison, optionally rank by `abs(log2fc)` and return annotation tables.
